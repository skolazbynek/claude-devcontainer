...
| `CLD_BROKER_ENDPOINT` | `stage_broker` -> master | Broker endpoint `[user@]host:port`; presence (with the mounted key) is what makes `cld broker` available in-container |
