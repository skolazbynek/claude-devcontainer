...
- `cld/messenger/mailbox.py` -- pure filesystem transport (atomic `tmp/` write + `rename()` into `inbox/`; `archive/`; append-only `outbox.log`). No MCP/Docker coupling: `list_containers()` delegates to `cld/host_docker.py` (local docker on host, broker inside master) and `resolve_recipient()` short-circuits to filesystem delivery when the recipient names an existing mailbox dir (the reply path -- so agents message back with no host channel). Unit-testable with `tmp_path`.
