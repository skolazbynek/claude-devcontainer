...
Every subcommand (`cld`, `master`, `agent`, `run`, `chain run`) shares one notion of an **anchor change**: an immutable revision from which all command-created changes descend. Default is the current change (`@`); override with `-r`/`--revision`. The host pins the anchor to a commit hash `A`, then calls `stage_anchor_with_scratch` (in `cld/vcs/scratch.py`) which writes any scratch files into `<repo>/.cld-run/` and runs `jj split --onto <A> -m "cld anchor: <session>" .cld-run` to produce a dedicated child commit `B` containing only `.cld-run/*`. `B`'s hash becomes `AGENT_ANCHOR_HASH`; the container entrypoint creates its ephemeral workspace on top of `B`. The user's original working-copy change stays where it was, minus the `.cld-run/` files. The in-container `vcs_assert_descendant` guard (in `vcs-lib.sh`) refuses to commit or squash if `@` no longer descends from `B`.

`.cld-run/` is a reserved, **not-gitignored** top-level directory. It appears only briefly on the host during scratch staging, then gets extracted into `B` by the split. Scratch content (task descriptions, personas, patches) is thus structurally rooted in the anchor tree and readable by the agent from its own workspace at `/workspace/current/.cld-run/*`.

The shared helpers now live in two files: `cld/vcs/anchor.py` (`resolve_anchor`, `assert_descendant`) and `cld/vcs/scratch.py` (`stage_anchor_with_scratch`). Anchor persistence across `cld master restart` / `cld agent restart` is provided by the jj bookmark named `<session>` in the origin's store: the container entrypoint detects it and reattaches by pointing a fresh workspace at the bookmark's last tip, so no host-side `.cld/anchors/` record is needed. `stage_anchor_with_scratch` is jj-only; the git backend raises `NotImplementedError`.
