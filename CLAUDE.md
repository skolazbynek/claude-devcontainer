...
Every subcommand (`cld`, `master`, `agent`, `run`, `task-agent start`) shares one notion of an **anchor change**: an immutable revision from which all command-created changes descend, and -- per the anchor descendant-tree contract -- the only boundary on what a container may touch: any change descending from the anchor is fair game, including changes that already existed before the container was spawned, not just ones the container itself creates. Default is the current change (`@`); override with `-r`/`--revision`. The host only *resolves* the base revision `A` (via `resolve_anchor_checked` in `cld/docker.py`, which wraps `resolve_anchor` in `cld/vcs/anchor.py` with the live-anchor overlap refusal described below); the origin working copy is never touched -- crucial for the common jj case where the user's `@` **is** `A`. Everything else runs **inside the peer container's ephemeral workspace** at `/workspace/current`: after `jj workspace add --name <session> -r <A> /workspace/current`, `python -m cld.vcs.scratch` writes the scratch payload under `.cld-run/*` and runs `jj commit -m "cld anchor: <session> mode=<mode>" .cld-run`, producing a scratch commit `B` (child of `A`, carrying only `.cld-run/*`) and advancing the workspace's `@` to a fresh empty child of `B` where the agent operates. The in-container `vcs_assert_descendant` guard (in `vcs-lib.sh`) refuses to commit or squash if `@` no longer descends from `AGENT_ANCHOR_HASH`.

**Two anchor modes** (`--shared-anchor`, default off, available on every launcher above): given a preexisting chain `A---B`,
- **isolated (default):** `AGENT_ANCHOR_HASH` is `B` itself, so touch rights are limited to `B`'s own descendant tree -- the container gets a fresh sibling of `A`'s other descendants and cannot touch them. This is the right mode for reviewers, implementations, and most task-agent work.
- **shared (`--shared-anchor`, explicit and meant to be human-approved):** `AGENT_ANCHOR_HASH` is `A` itself, so any pre-existing descendant of `A` (including `B` and its descendants) is inside the container's editable tree. Meant for consolidation/cleanup work across a change tree built by several agents, before handing off to review or pushing.

Both modes are refused by `resolve_anchor_checked` if they would overlap another **live** container's reach (its own anchor's descendant tree), scoped host-wide across every running `cld` container (any role), not just one master's fleet: (1) always -- anchoring inside a live container's own stack is refused, since it can rewrite that stack at any moment; (2) shared mode only -- claiming a tree that already has a live occupant somewhere inside it is refused, since a shared anchor's reach covers all of that tree. Two isolated siblings spawned off the same base are unaffected by either check. The anchor and its mode are stamped as host-set `org.cld.anchor`/`org.cld.anchor-mode` docker labels at container creation (`docker_anchor_list` in `cld/docker.py` reads them back for the overlap check and for `cld task-agent status`'s detail view); the mode is also embedded in scratch commit `B`'s description so a restart/reattach recovers the right hash.
...
- `AGENT_ANCHOR_MODE` -- `"isolated"` (default) or `"shared"`, set by `anchor_env_args`.
- `AGENT_ANCHOR_HASH` is **not** sent by the host; it is computed peer-side after the workspace exists (`B` or `A` depending on `AGENT_ANCHOR_MODE`) and exported by the entrypoint for downstream consumers (agent-loop, descendant guard, `summary.json`).

Shared helpers: `cld/vcs/anchor.py` (`resolve_anchor`, `assert_descendant`), `cld/docker.py` (`resolve_anchor_checked`, `docker_anchor_list`) and `cld/vcs/scratch.py` (`stage_in_workspace`, `stage_from_env`, envelope encode/decode). Anchor persistence across `cld master restart` / `cld agent restart` is provided by the jj bookmark named `<session>` in the origin's store: the container entrypoint detects it and reattaches by pointing a fresh workspace at the bookmark's last tip. On reattach, `AGENT_ANCHOR_HASH` is recovered by walking the bookmark's ancestors for the commit whose description matches `"cld anchor: <session>*"` (that finds scratch commit `B`), reading its mode back out of that same description, and using `B` itself (isolated) or its parent `A` (shared). The peer-staging code path is jj-only; the git backend has no equivalent.
